...
class CGLMMRAgent:
    def __init__(self, model, backend, retrieve_k, temperature_c5, sglang_host="http://localhost", sglang_port=30000):
        # Initialize the CGLM-MR architecture instead of AgenticMemorySystem
        self.memory_system = CGLMMRSystem(
            model_name='all-MiniLM-L6-v2',
            llm_backend=backend,
            llm_model=model,
            sglang_host=sglang_host,
            sglang_port=sglang_port,
            evo_threshold=100 # Default threshold for synaptic pruning
        )
    ...
    def retrieve_memory(self, query, k=10):
        # The CGLM-MR architecture intrinsically uses the Meta-Reasoning execution controller 
        # (System 1 + System 2 Dual Process Theory) inside retrieve_memory.
        return self.memory_system.retrieve_memory(query, k=k)
    ...
    def answer_question(self, question: str, category: int, answer: str) -> tuple:
        """Generate answer for a question given the Cognitive Graph context."""
        keywords = self.generate_query_llm(question)
        
        # Meta-reasoning controller handles System 1 vs System 2 under the hood
...
Answer the question based only on the information provided in the context above."""
        ...
        elif category == 2: # Temporal reasoning
            user_prompt = f"""
...
def setup_logger(log_file: Optional[str] = None) -> logging.Logger:
    ...
    logger = logging.getLogger('cglm_mr_eval')
...
def evaluate_dataset(dataset_path: str, model: str, output_path: Optional[str] = None, ratio: float = 1.0, backend: str = "sglang", temperature_c5: float = 0.5, retrieve_k: int = 10, sglang_host: str = "http://localhost", sglang_port: int = 30000):
    """Evaluate the CGLM-MR agent on the LoComo dataset."""
    
    timestamp = datetime.now().strftime("%Y-%m-%d-%H-%M")
    log_filename = f"eval_cglm_mr_{model}_{backend}_ratio{ratio}_{timestamp}.log"
    ...
    memories_dir = os.path.join(os.path.dirname(__file__), "cached_memories_cglm_mr_{}_{}".format(backend, model))
    os.makedirs(memories_dir, exist_ok=True)
    allow_categories = [1,2,3,4,5]
    
    for sample_idx, sample in enumerate(samples):
        agent = CGLMMRAgent(model, backend, retrieve_k, temperature_c5, sglang_host, sglang_port)
        ...
        if os.path.exists(memory_cache_file):
            logger.info(f"Loading cached CGLM-MR memories for sample {sample_idx}")
            ...
            logger.info(f"Successfully loaded {len(cached_memories)} memory nodes")
        else:
            logger.info(f"No cached memories found for sample {sample_idx}. Constructing Cognitive Graph...")
...
Processing sample {sample_idx + 1}/{len(samples)}")
        ...
        for qa in sample.qa:
            if int(qa.category) in allow_categories:
                ...
                prediction_raw, user_prompt, raw_context = agent.answer_question(qa.question, qa.category, qa.final_answer)
                try:
                    prediction = json.loads(prediction_raw)["answer"]
                except:
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate CGLM-MR agent on LoComo dataset")
    ...
    parser.add_argument("--ratio", type=float, default=1.0, help="Ratio of dataset to evaluate (0.0 to 1.0)")
    parser.add_argument("--backend", type=str, default="sglang", help="Backend to use (openai, ollama, or sglang)")
    parser.add_argument("--temperature_c5", type=float, default=0.5, help="Temperature for adversarial questions")
    parser.add_argument("--retrieve_k", type=int, default=10, help="Retrieve k")
    parser.add_argument("--sglang_host", type=str, default="http://localhost", help="SGLang server host")
